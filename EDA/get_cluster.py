...
def get_args():
    ...
    parser.add_argument(
        "--item_data_path", default="similarity_item_99243.csv", type=str
    )
    parser.add_argument(
        "--train_data_path", default="/opt/ml/input/data/hi_interaction.csv", type=str
    )
    parser.add_argument(
        "--similarity_list_path",
        default="/opt/ml/input/data/similarity_99243.pickle",
        type=str,
    )
...
if __name__ == "__main__":
    ...
    item_["similar_item_list"] = item_.similarity_list.apply(
        lambda x: get_item(x, args.threshold)
    )
