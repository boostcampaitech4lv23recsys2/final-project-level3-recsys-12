...
def get_house_style():
    ...
    for cate_name in Style[1:]:
        with database.session_maker() as session:
            stmt = (
                select(House.house_id, House.style)
                .where(House.style == cate_name)
                .limit(1)
            )
...
def check_is_prefer(member_email, item_id):
    with database.session_maker() as session:
        ...
        stmt = (
            "select * from member_prefer where member_email='%s' and item_id='%d'"
            % (member_email, item_id)
        )
...
def delete_member_prefer(member_email, item_id):
    with database.session_maker() as session:
        ...
        stmt = "delete from member_prefer where member_email='%s' and item_id='%d'" % (
            member_email,
            item_id,
        )
        if not stmt:
...
def get_inference_result(member_email):
    with database.session_maker() as session:
        ...
        stmt = "select item_id from inference_result where member_email='%s'" % (
            member_email
        )
        return session.execute(stmt).fetchall()
