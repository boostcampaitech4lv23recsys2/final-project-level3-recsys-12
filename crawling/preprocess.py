...
class Main:
    ...
    def filter_by_item_category(self):
        ...
        self.item = self.item[
            (~self.item.category.str.contains("|".join(del_list)))
            & ~(self.item.category == "")
        ]

        self.house_item = self.house_item[
            self.house_item.item.isin(self.item.item.unique())
        ]
        self.item = self.item[self.item.item.isin(self.house_item.item.unique())]

        self.house_code = self.house_code[
            self.house_code.house.isin(self.house_item.house.unique())
        ]
        self.house_card = self.house_card[
            self.house_card.house.isin(self.house_item.house.unique())
        ]
        self.card = self.card[self.card.card.isin(self.house_card.card.unique())]
        self.house_floor_color = self.house_floor_color[
            self.house_floor_color.house.isin(self.house_item.house.unique())
        ]
        self.house_main_color = self.house_main_color[
            self.house_main_color.house.isin(self.house_item.house.unique())
        ]
        self.house_wall_color = self.house_wall_color[
            self.house_wall_color.house.isin(self.house_item.house.unique())
        ]
    ...
    def preprocessing(self):
        ...
        self.clusterd_item, self.cluster_major_item = merge_cluster(
            _item.item, self.house_item, self.args
        )
    ...
    def make_csv(self):
        ...
        self.preprocessed_house.to_csv(
            self.output_path + "house.tsv", sep="\t", index=False
        )
        self.preprocessed_item.to_csv(
            self.output_path + "item.tsv", sep="\t", index=False
        )
        self.preprocessed_card.to_csv(
            self.output_path + "card.tsv", sep="\t", index=False
        )
        self.house_item.to_csv(
            self.output_path + "house_item.tsv", sep="\t", index=False
        )
        self.house_card.to_csv(
            self.output_path + "house_card.tsv", sep="\t", index=False
        )
        self.house_code.to_csv(
            self.output_path + "house_code.tsv", sep="\t", index=False
        )
        self.house_floor_color.to_csv(
            self.output_path + "house_floor_color.tsv", sep="\t", index=False
        )
        self.house_main_color.to_csv(
            self.output_path + "house_main_color.tsv", sep="\t", index=False
        )
        self.house_wall_color.to_csv(
            self.output_path + "house_wall_color.tsv", sep="\t", index=False
        )
