...
global del_list, CLUSTER_ID, NON_CLUSTER_ID
CLUSTER_ID = 0
NON_CLUSTER_ID = -1
...
def get_cluster_by_BFS(df:pd.DataFrame, args):
    global CLUSTER_ID, NON_CLUSTER_ID
    
    def BFS(dfid:int, cluster_id):
        q = deque([(dfid, 0)])
        visited[dfid] = cluster_id
        while(q):
            node, hop = q.popleft()
    ...
    df["cluster_id"] = df.item.map(visited)
    
    df.loc[df.preprocessed_title.str.contains("etc|ETC"), "cluster_id"] = -1
    ...
    for dfid in not_clustered_item:
        ...
        if len(CNT) >= 2:
            ...
            if tmp[0][0] == -1: # 가장 빈도 높은 클러스터가 클러스터링이 안되어있을 때 : -1
                final_cluster_id = tmp[1][0] # 나머지 모든 item을 2등 최빈 클러스터로 묶어줌
                for neighbor_dfid in graph[dfid] + [dfid]:
                    visited[neighbor_dfid] = final_cluster_id
            else:
                final_cluster_id = tmp[0][0] # 나머지 모든 item을 1등 최빈 클러스터로 묶어줌
                for neighbor_dfid in graph[dfid] + [dfid]:
                    visited[neighbor_dfid] = final_cluster_id
        elif len(CNT) == 1:
            tmp = CNT.most_common(1)
            if tmp[0][0] == -1: # 클러스터가 클러스터링이 안되어있을 때 : -1
                final_cluster_id = tmp[1][0] # 나머지 모든 item을 not_clustered_item의 클러스터로 묶어줌
                for neighbor_dfid in graph[dfid] + [dfid]:
                    visited[neighbor_dfid] = final_cluster_id
            else:
                final_cluster_id = tmp[0][0] # 나머지 모든 item을 1등 최빈 클러스터로 묶어줌
