...
class ClusterItem():
    ...
    def preprocessing(self):
        self.cluster_major_item = self.cluster_major_item.rename(columns={"cluster_id":"cluster_id"})
...
def merge_cluster(item, train, args):
    ...
    cluster_major_item_id = item.rename(columns={"item":"major_item"}).groupby("cluster_id")["major_item"].apply(lambda x:list(x)[0]).reset_index()
...
def convert_non_cluster(df:pd.DataFrame):
    c = df.cluster_id.max()
    cluster_ids_for_non_cluster = [i + c + 1 for i in range(len(df[df.cluster_id == -1]))]
    df.loc[df.cluster_id == -1, "cluster_id"] = cluster_ids_for_non_cluster
    item2cluster = {item_id:cluster_id for item_id, cluster_id in zip(df.item, df.cluster_id)}
