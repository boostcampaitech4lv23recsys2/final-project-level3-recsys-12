...
class Item:
    ...
    def process_title(self):
        ...
        self.item.preprocessed_title = self.item.preprocessed_title.str.replace(
        ...
        self.item.preprocessed_title = self.item.preprocessed_title.progress_apply(
            lambda x: tokenize(x)
        )
        print(
            f"###########################    item preprocessing: PHASE_1_basic_preprocess    ###########################"
        )
        self.item.to_csv(
            self.args.output_path + f"PHASE_1_basic_preprocess_item.tsv",
            sep="\t",
            index=False,
        )
    ...
    def add_similarity(self):
        ...
        print(
            f"###########################    item preprocessing: PHASE_2_add_similarity    ###########################"
        )
        self.item.to_csv(
            self.args.output_path + f"PHASE_2_add_similarity_item.tsv",
            sep="\t",
            index=False,
        )
    ...
    def add_cluster(self):
        ...
        self.item["similar_dflist"] = self.item.similarity_list.apply(
            lambda x: get_item(x, self.args.clustering_threshold)
        )
        self.item = get_cluster_by_BFS(self.item, self.args)
        self.item.drop(columns=["similar_dflist"], inplace=True)
        print(
            f"###########################    item preprocessing: PHASE_3_add_cluster    ###########################"
        )
        self.item.to_csv(
            self.args.output_path + f"PHASE_3_add_cluster_item.tsv",
            sep="\t",
            index=False,
        )
