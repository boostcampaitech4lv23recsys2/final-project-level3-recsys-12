...
def get_similarity_list(df:pd.DataFrame, args):
    ...
    for i, (id, t) in tqdm(enumerate(zip(df.item[:len(df) - window + 1], df.preprocessed_title[:len(df) - window + 1]))):
        word = t
        tmp = []
        for j, (id_, t_) in enumerate(zip(df.item[i + 1:i + window], df.preprocessed_title[i + 1:i + window])):
            compare = t_
            if mode == 1:
    ...
    for i, (id, t) in tqdm(enumerate(zip(df.item[start:end], df.preprocessed_title[start:end]))):
        word = t
        tmp = []
        for j, (id_, t_) in enumerate(zip(df.item[i + start + 1:end], df.preprocessed_title[i + start + 1:end])):
            compare = t_
            if mode == 1:
                similarity = gmp_similarity(word, compare)
            elif mode == 0:
