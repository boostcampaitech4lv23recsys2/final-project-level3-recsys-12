...
class CrawlingManager(TodaysHome):
    ...
    def __init__(self, args):
        ...
        self.hi_interaction_path = os.path.join(
            self.args.datapath, "hi_interaction.csv"
        )
        self.hc_interaction_path = os.path.join(
            self.args.datapath, "hc_interaction.csv"
        )
        ...
        self.floor_color_path = os.path.join(
            self.args.datapath, "house_floor_color.csv"
        )
    ...
    def update_house_code(self):
        ...
        if self.args.just_update:
            updated_house = self.crawler.scroll_down(
                class_name=self.house_in_projects_class, n_iter=self.args.n_iter
            )
        else:
            updated_house = self.crawler.scroll_down(
                class_name=self.house_in_projects_class
            )
        new = pd.DataFrame(set(map(int, updated_house)), columns=["house"])
        pd.concat([self.original_house_code, new]).drop_duplicates().sort_values(
            "house"
        ).to_csv(self.house_code_path, index=False)
        self.original_house_code = pd.read_csv(self.house_code_path)
    ...
    def update_hi_interaction(self):
        ...
        setted = set(
            map(
                ...
                os.listdir(self.hi_interaction_folder),
            )
        )
        founded_house = entire_house_code - setted
        iterator = (
            tqdm(founded_house, "hi_interaction")
            if self.args.just_update
            else tqdm(entire_house_code, "hi_interaction")
        )
        for housecode in iterator:
            file_name = os.path.join(
                self.hi_interaction_folder, f"house_item_interaction_{housecode}.csv"
            )
            updated_data = []
            self.crawler.get_page(self.crawler.get_project_use_products_url(housecode))
            items = map(
                int,
                self.crawler.scroll_down(class_name=self.item_in_user_products_class),
            )
            for item in items:
            ...
            if housecode in setted:
                ...
                pd.concat([origin, new_data]).drop_duplicates().sort_values(
                    "item"
                ).to_csv(file_name, index=False)
            else:
                new_data.drop_duplicates().sort_values("item").to_csv(
                    file_name, index=False
                )
        merge_all_in_dir(self.hi_interaction_folder).sort_values(
            ["house", "item"]
        ).to_csv(self.hi_interaction_path, index=False)
        self.original_hi_interaction = pd.read_csv(self.hi_interaction_path)
    ...
    def update_hc_interaction(self):
        ...
        setted = set(
            map(
                lambda x: int(x.split("_")[-1].split(".")[0]),
                os.listdir(self.hc_interaction_folder),
            )
        )
        founded_house = entire_house_code - setted
        iterator = (
            tqdm(founded_house, "hc_interaction")
            if self.args.just_update
            else tqdm(entire_house_code, "hc_interaction")
        )
        for housecode in iterator:
            if housecode in setted:
                continue
            file_name = os.path.join(
                self.hc_interaction_folder, f"house_card_interaction_{housecode}.csv"
            )
            updated_data = []
            self.bs4_crawler.get_page(self.bs4_crawler.get_project_url(housecode))
            if self.bs4_crawler.page:
                card_elements = self.bs4_crawler.page.find_all(
                    "a", "project-detail-image-block__link"
                )
                for card_data in card_elements:
                    card = int(card_data.attrs["href"].split("/")[-1])
                    updated_data.append([housecode, card])
            pd.DataFrame(updated_data, columns=["house", "card"]).to_csv(
                file_name, index=False
            )
        merge_all_in_dir(self.hc_interaction_folder).sort_values(
            ["house", "card"]
        ).to_csv(self.hc_interaction_path, index=False)
        self.original_hc_interaction = pd.read_csv(self.hc_interaction_path)
    ...
    def update_house(self):
        ...
        new = {}
        iterator = (
            tqdm(founded_house, "house")
            if self.args.just_update
            else tqdm(entire_house_code, "house")
        )
        for house in iterator:
            ...
            if data:
                new[house] = data
            new_df = (
                pd.DataFrame.from_dict(new, orient="index")
                .reset_index()
                .rename(columns={"index": "house"})
            )
            pd.concat([self.original_house, new_df]).drop_duplicates().sort_values(
                "house"
            ).to_csv(self.house_path, index=False)
    ...
    def update_item(self):
        ...
        new = {}
        iterator = (
            tqdm(founded_item, "item")
            if self.args.just_update
            else tqdm(entire_item_code, "item")
        )
        for item in iterator:
            ...
            pd.concat([self.original_item, new_df]).drop_duplicates().sort_values(
                "item"
            ).to_csv(self.item_path, index=False)
    ...
    def update_card(self):
        ...
        for card in iterator:
            ...
            new_df = (
                pd.DataFrame.from_dict(new, orient="index")
                .reset_index()
                .rename(columns={"index": "card"})
            )
            pd.concat([self.original_card, new_df]).drop_duplicates().sort_values(
                "card"
            ).to_csv(self.card_path, index=False)
...
def save_color(manager):
    ...
    pd.DataFrame.from_dict(
        manager.get_color("main"), orient="index"
    ).reset_index().rename(columns={"index": "house"}).to_csv(
        manager.main_color_path, index=False
    )
    pd.DataFrame.from_dict(
        manager.get_color("wall"), orient="index"
    ).reset_index().rename(columns={"index": "house"}).to_csv(
        manager.wall_color_path, index=False
    )
    pd.DataFrame.from_dict(
        manager.get_color("floor"), orient="index"
    ).reset_index().rename(columns={"index": "house"}).to_csv(
        manager.floor_color_path, index=False
    )
